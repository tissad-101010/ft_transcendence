...
class AuthService {
    constructor(app) {
        this.refreshTimeout = 7 * 24 * 60 * 60; // 7 days in seconds
        this.accessTimeout = 15 * 60; // 15 minutes in seconds
        this.prismaClient = app.prisma;
        this.redisClient = app.redis;
        this.userService = new users_services_1.UsersService(this.prismaClient);
    }
    // register a new user
    async registerUser(inputData) {
        console.log('[Signup authservice] Registering user:', inputData.username);
        // hash the password before storing
        const hashedPassword = await crypt_utils_1.CryptUtils.hashLongPassword(inputData.password);
    ...
    async authenticateUser(inputData) {
        ...
        // verify password  
        const isPasswordValid = await crypt_utils_1.CryptUtils.verifyLongPassword(inputData.password, user.passwordHash);
        if (!isPasswordValid) {
            return {
        ...
        if (isTwoFactorEnabled) {
            const tempToken = jwt_utils_1.JwtUtils.generateTwoFactorTempToken(loginResponse);
            if (!tempToken) {
        ...
        await this.redisClient.set(`refresh_token:${refreshToken}`, user.id, 'EX', 60 * 60 * 24 * 7);
        // store access token in redis cache (optional)
        await this.redisClient.set(`access_token:${user.id}`, accessToken, 'EX', this.accessTimeout);
    ...
    async changeUserPassword(userId, currentPassword, newPassword) {
        ...
        const isPasswordValid = await crypt_utils_1.CryptUtils.verifyLongPassword(currentPassword, user.passwordHash);
        if (!isPasswordValid) {
        ...
        // hash new password
        const hashedNewPassword = await crypt_utils_1.CryptUtils.hashLongPassword(newPassword);
