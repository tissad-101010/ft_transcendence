    ...
    try {
      ...
      const friendlyMatches = await (fastify.prisma as any).friendlyMatch.findMany({
        ...
        include: {
          ...
          player2: { select: { id: true, login: true } },
        },
      ...
      fastify.log.error(error, 'Erreur récupération matches utilisateur');
      const normalizedError = error instanceof Error ? error : new Error(String(error));
      return reply.code(500).send({
        success: false,
        message: normalizedError.message || 'Erreur lors de la récupération des matches',
      });
    }
  });
